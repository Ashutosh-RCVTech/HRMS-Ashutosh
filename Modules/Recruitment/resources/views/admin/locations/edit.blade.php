...
@section('content')
    <main class="w-full px-6 pb-6 pt-24 dark:bg-slate-900 dark:text-white">
        <div class="container mx-auto px-4 py-8">
            <div class="p-6 bg-white border-b border-gray-200 dark:bg-slate-900 dark:text-white rounded-xl shadow-lg">
                <h1 class="text-2xl font-bold mb-8">Edit Location</h1>

                <form id="location_edit_form" method="POST" action="{{ route('admin.locations.update', $location->id) }}">
                    @csrf
                    @method('PUT')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const form = document.getElementById('location_edit_form');
                ...
                fetch(this.action, {
                    ...
                    .then(data => {
                        if (data.errors) {
                            ...
                            toastr.error(errorMsg || 'Failed to update location');
                        } else {
