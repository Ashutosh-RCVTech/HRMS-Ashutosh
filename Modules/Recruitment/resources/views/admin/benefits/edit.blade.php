...
@section('content')
    <main class="w-full px-6 pb-6 pt-24 dark:bg-slate-900 dark:text-white">
        <div class="container mx-auto px-4 py-8">
            <div class="p-6 bg-white border-b border-gray-200 dark:bg-slate-900 dark:text-white">
                <h1 class="text-2xl font-bold mb-8">Edit Benefit</h1>

                <form method="POST" id="benefit_edit_form" action="{{ route('admin.benefits.update', $benefit->id) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-6">
                        ...
                        <input id="name" name="name" type="text" value="{{ old('name', $benefit->name) }}"
                            required
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            $("#benefit_edit_form").submit(function(e) {
                e.preventDefault();
                let form = $(this)[0];
                let data = new FormData(form);

                $.ajax({
                    url: "{{ route('admin.benefits.update', $benefit->id) }}",
                    type: "POST",
                    data: data,
                    dataType: "JSON",
                    processData: false,
                    ...
                    success: function(response) {
                        if (response.errors) {
                            ...
                            toastr.error(errorMsg || 'Failed to update Benefit');
                        } else {
