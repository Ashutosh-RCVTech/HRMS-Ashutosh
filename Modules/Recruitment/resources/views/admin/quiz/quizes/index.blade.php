            ...
            <div class="container mx-auto px-4 py-8">
                <div class="p-6 bg-white border-b border-gray-200 dark:bg-slate-900 dark:text-white">
                    ...
                    <div class="flex justify-between items-center mb-8">
                        ...
                        <a href="{{ route('admin.quiz.quizes.create', $id) }}"
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 dark:bg-slate-800 dark:divide-gray-700">
                                @forelse ($quizes as $quiz)
                                    <tr class="hover:bg-gray-400 dark:hover:bg-gray-700 transition-colors">
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-900 dark:text-white">{{ $quiz->name }}</td>
                                        <td class="px-6 py-4 text-sm text-gray-900 dark:text-white">{{ $quiz->marks }}</td>
                                        <td class="px-6 py-4 text-sm text-gray-900 dark:text-white">
                                            {{ $quiz->passing_marks }}
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-900 dark:text-white">{{ $quiz->duration }}
                                        </td>
                                        {{-- <td class="px-6 py-4 text-sm text-gray-900 dark:text-white">
                                            {{ $quiz->schedule_date }}
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-900 dark:text-white">{{ $quiz->start_time }}
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-900 dark:text-white">{{ $quiz->timezone }}
                                        </td> --}}
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex space-x-2">
                                                <a href="{{ route('admin.quiz.courses.quizes.basic.info.edit', $quiz->id) }}"
                                                ...
                                                <a href="{{ route('admin.quiz.quizes.show', $quiz->id) }}"
                                                    ...
                                                    <svg fill="#eee" class="w-4 h-4 mr-2 dark:text-white"
                                                ...
                                                <a href="{{ route('admin.quiz.quizes.show.results', ['quizId' => $quiz->id, 'status' => 1]) }}"
                                                    class="inline-flex items-center px-3 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:outline-none focus:border-indigo-700 focus:ring focus:ring-indigo-200 active:bg-indigo-800 transition ease-in-out duration-150">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                        viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                                        class="w-4 h-4 mr-2">
                                                ...
                                                <form action="{{ route('admin.quiz.quizes.destroy', $quiz->id) }}"
