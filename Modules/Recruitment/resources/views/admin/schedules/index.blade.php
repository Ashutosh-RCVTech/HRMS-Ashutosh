                    ...
                    <form id="search-form" class="flex gap-4">
                        <div class="flex-1">
                            <input type="text" name="search" id="search" 
                                ...
                                placeholder="Search schedules..." value="{{ request('search') }}">
                        </div>
                        <button type="submit" 
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-slate-800 dark:divide-gray-700">
                            @forelse ($schedules as $schedule)
                                <tr class="hover:bg-gray-400 dark:hover:bg-gray-700 transition-colors">
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-900 dark:text-white">{{ $schedule->name }}</td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            searchForm.addEventListener('submit', function(e) {
                ...
                if (searchValue) {
                    window.location.href = `{{ route('admin.schedules.index') }}?search=${encodeURIComponent(searchValue)}`;
                } else {
