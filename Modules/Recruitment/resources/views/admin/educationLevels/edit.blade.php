...
@section('content')
    <main class="w-full px-6 pb-6 pt-24 dark:bg-slate-900 dark:text-white">
        <div class="container mx-auto px-4 py-8">
            <div class="p-6 bg-white border-b border-gray-200 dark:bg-slate-900 dark:text-white">
                <h1 class="text-2xl font-bold mb-8">Edit Education Level</h1>

                <form id="educationLevels_edit_form" method="POST"
                    action="{{ route('admin.educationLevels.update', $educationLevel->id) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-6">
                        ...
                        <input id="name" name="name" type="text" value="{{ old('name', $educationLevel->name) }}"
                            required
                    ...
                    <div class="flex items-center justify-between mt-8">
                        ...
                        <a href="{{ route('admin.educationLevels.index') }}"
                            class="ml-3 inline-flex items-center px-4 py-2 bg-gray-300 border border-transparent 
                              rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest 
                              hover:bg-gray-400 active:bg-gray-500 focus:outline-none 
                              focus:border-gray-500 focus:ring ring-gray-300 
                              disabled:opacity-25 transition ease-in-out duration-150">
                            Cancel
                        </a>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const form = document.getElementById('educationLevels_edit_form');
                ...
                fetch(this.action, {
                    ...
                    .then(data => {
                        if (data.errors) {
                            ...
                            toastr.error(errorMsg || 'Failed to update education level');
                        } else {
