        ...
        <div class="container mx-auto px-4 py-8">
            <div class="p-6 bg-white border-b border-gray-200 dark:bg-slate-900 dark:text-white rounded-xl shadow-lg">
                ...
                <div class="flex justify-between items-center mb-8">
                    <h1 class="text-2xl font-bold">Education Levels</h1>
                    <a href="{{ route('admin.educationLevels.create') }}"
                    ...
                    <form id="search-form" class="flex gap-4">
                        <div class="flex-1">
                            <input type="text" name="search" id="search"
                                ...
                                placeholder="Search education levels..." value="{{ request('search') }}">
                        </div>
                        <button type="submit"
                            ...
                            @forelse ($educationLevels as $educationLevel)
                                <tr class="hover:bg-gray-400 dark:hover:bg-gray-700 transition-colors">
                                    <td class="px-6 py-4 text-sm text-gray-900 dark:text-white">
                                        {{ ($educationLevels->currentPage() - 1) * $educationLevels->perPage() + $loop->iteration }}
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-900 dark:text-white">{{ $educationLevel->name }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex space-x-2">
                                            <a href="{{ route('admin.educationLevels.edit', $educationLevel->id) }}"
                                            ...
                                            <form action="{{ route('admin.educationLevels.destroy', $educationLevel->id) }}"
            ...
            searchForm.addEventListener('submit', function(e) {
                ...
                if (searchValue) {
                    window.location.href =
                        `{{ route('admin.educationLevels.index') }}?search=${encodeURIComponent(searchValue)}`;
                } else {
                    window.location.href = `{{ route('admin.educationLevels.index') }}`;
                }
            });
        });
    </script>
